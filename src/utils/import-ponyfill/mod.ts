import type { DenoConfigurationFile } from "./deps.ts";
...
async function getDenoConfiguration() {
  for (
    const posibleDenoConfigurationURL of posibleDenoConfigurationURLs!
  ) {
    try {
      return JSON.parse(
...
>(
  ...
  try {
    ...
    const base = new URL(import.meta.url); //ErrorStackParser.parse(new Error())[1].fileName;
